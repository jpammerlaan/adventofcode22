...
def print_map(walls, blizzards, blizzard_cycles, i, pos):
    x_coords, y_coords = zip(*walls.keys())
    x_min, x_max = min(x_coords), max(x_coords) + 1
    y_min, y_max = min(y_coords), max(y_coords) + 1
    blizzard_lookup = {v: k for k, v in BLIZZARD_DIRECTIONS.items()}
    current_blizzards = [blizzard[i % len(blizzard)] for blizzard in blizzard_cycles]
    for y in range(y_min, y_max):
        for x in range(x_min, x_max):
            if (x, y) in walls.keys():
                print('#', end='')
            elif len(blizz := [(i, p) for i, p in enumerate(current_blizzards) if (x, y) == p]):
                if len(blizz) == 1:
                    i, _ = blizz.pop()
                    print(blizzard_lookup[blizzards[i][1]], end='')
                else:
                    print(len(blizz), end='')
            elif (x, y) == pos:
                print('E', end='')
...
def part_two(walls, blizzards):
    ...
    num_rounds = bfs(walls, blizzards, start=start, end=end, start_tick=652)
    return num_rounds
